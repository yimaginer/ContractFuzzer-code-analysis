...
import (
	"bufio"
	abi_gen "contract_fuzzer/src/ContractFuzzer/abi"
	"fmt"
	"io"
	"log"
	"net/http"
...
// 一个典型的 .bin.sig 文件内容如下：
// a9059cbb:transfer(address,uint256)
// 70a08231:balanceOf(address)
// dd62ed3e:allowance(address,address)
// 095ea7b3:approve(address,uint256)
// 23b872dd:transferFrom(address,address,uint256)
// 把合约内部调用签名列表，将其添加到全局变量 G_current_bin_fun_sigs 中
...
// 从 abi 读取字符串到签名的映射 存储到全局变量 G_current_abi_sigs 中
...
func Start(dir string, outdir string) error {
	...
	if len(Global_contractList) == 0 {
		...
		for i := Global_fstart; i < Global_fend && i < len(files)-1; i++ {
			...
			setG_current_bin_fun_sigs() // 解析 .bin.sig 文件存到 G_current_bin_fun_sigs
			setG_current_abi_sigs()     // 解析 .abi 文件存到 G_current_abi_sigs
